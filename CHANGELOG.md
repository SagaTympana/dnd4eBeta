...
* __Status-Aware Automation:__ Universal penalties to skills and movement are now applied automatically by conditions like slowed, immobilised, and blinded. You no longer need to use an Active Effect with separate modifiers; just apply the status condition, either in an AE or from the token UD, and the penalties will be handled by the system. (Fox)
  * A new setting, "Dynamic Automation", has been introduced to control this behaviour (default ON). Deactivating it will disable not only the new condition-based automation, but also automation related to position, terrain, and so forth. Basically, for those who want automation only via Active Effects and/or Macros.
  *  Dynamic Automation includes and supercedes the existing "Mark Automation" setting. 
  * The Example Effects compendium has been updated to reflect these changes.
* __Improved Movement Handling__ (Fox)
  ...
  * Speed calcs now factor in movement-limiting conditions when using Dynamic Automation. The actor's sheet will display their current speeds correctly, rather than only the ruler reflecting these changes.
  * Checkboxes have been added for actors who Squeeze and/or Crawl at full speed
  * "Charging" has been added to standard conditions. This is pretty unnecessary since we use the attack roll itself to handle charging bonuses, but it does allow automation of, say, +def vs opportunity attacks while charging.
  * "Flying" has been moved into legacy conditions, on the basis that it's handled by movement mode.
* __Obscured Terrain__ (SagaTympana)
  * "Obscured Terrain" region behavior is now available, with filters for disposition, creature origin, and creature type.
  * Concealment is calculated from a combination of the attacker's senses, Obscured Terrain regions, and the light level on the target token. As always, calculated concealment is merely a suggestion and can be overridden in the attack roll dialog.
* __Sync Senses to Token Vision:__ This new system setting is on by default, allowing a token's vision to be inferred automatically based on the token's actor's senses. For fine-grained control, token config takes predence over inferred settings. (SagaTympana)
* A blue border has been added around the temp HP overlaid on the token actor HP bar, to make it a bit easier to see on characters at full HP. (Fox)
### Bug Fixes & Under the Hood
* Fixed a bug where macros generated by dragging a power to the hotbar would no longer work (SagaTympana)
* Fixed a bug with TAH Integration and FF Saving Throw (Draconas)
...
* Updated system documentation (Fox)
