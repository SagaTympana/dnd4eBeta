...
export default class ActorSheet4e extends foundry.applications.api.HandlebarsApplicationMixin(foundry.applications.sheets.ActorSheetV2) {
   ...
   */
	...
	async _prepareContext(options) {
		...
		for (let i of context.items) {
			...
			if ((item.type === "power") && item.system.autoGenChatPowerCard) {
				...
				let detailsText = utils.preparePowerCardData(i.chatData, actorData, attackBonus);
	...
	_prepareItems(data) {
		...
		data.ritualPages = this.actor.itemTypes.ritual.reduce((pages, r) => pages + ((r.system.subtype === "ritual") && r.system.level) || 0, 0);
   ...
   */
	...
	*/
	static #onHPOptions(event, target) {
		...
		new apps.HPOptions({ document: this.actor }).render(true);
	}
	...
	*/
	...
	static #onSkillBonus(event, target) {
		...
		new apps.AttributeBonusDialog(options).render(true);
	}
	/* -------------------------------------------- */
	...
	static #onMovementDialog(event, target) {
		if (!this.actor.isOwner) return;
		event.preventDefault();
		new apps.MovementDialog({ document: this.actor }).render(true);
	}

	static #onConBonConfig(event, target) {
		if (!this.actor.isOwner) return;
		event.preventDefault();
		new apps.ConBonConfig({ document: this.actor }).render(true);
	}
	...
	static #onEncumbranceDialog(event, target) {
		if (!this.actor.isOwner) return;
		event.preventDefault();
		new apps.EncumbranceDialog({ document: this.actor }).render(true);
	}
	...
	static #onCustomRolldDescriptions(event, target) {
		...
		new apps.CustomRolldDescriptions({ document: this.actor }).render(true, options);
	}
	/**
   ...
   */
	static #onTraitSelector(event, target) {
		...
		new apps.TraitSelector({ document: this.actor, ...options }).render(true);
	}
   ...
   */
	...
	_onDropStackConsumables(itemData) {
		...
		utils.debugLog(similarItem.system.quantity);
		utils.debugLog(itemData.system.quantity);
