import { d20Roll, damageRoll, getAttackRollBonus } from "../helpers/dice.mjs";
...
export default class Item4e extends BaseDocumentMixin(foundry.documents.Item) {
	...
	async _preUpdate(changed, options, user) {
		...
		if (this.system.type === "consumable") {
			...
			if (system.damage.parts?.length > 0) {
				utils.debugLog("DnD4e: Updating an obsolete consumable that somehow still had a parts roll");
			...
			if (system.oldConsumableNeedsUpdate === true) {
				utils.debugLog("DnD4e: Updating an obsolete consumable");
				foundry.utils.setProperty(changed, "system.damage.parts", []);
		...
		if (this.system.type === "ritual") {
			...
			if (system.oldRitualNeedsUpdating === true) {
				utils.debugLog("DnD4e: Updating an obsolete ritual");
	...
	get preparedMaxUses() {
		...
		if (this.isOwned && !Number.isNumeric(max)) {
			...
			max = utils.evaluateFormula(max, this.actor?.getRollData(), { strict: true, contextName: "preparedMaxUses" });
			if (!max) return null;
		}
	 ...
	 */
	get hasTarget() {
		return !["none", ""].includes(this.system.target);
	}
	...
	get keywords() {
		...
		try {
			if (!("damageType" in this.system) && !("effectType" in this.system) && !this.system?.keywordsCustom) return { system: {}, custom: {}, string: "" };
	...
	getDamageType() {
		if ((this.type == "power") && this.actor) {
			...
			if (weapon && (!Array.from(Object.entries(this.system.damageType)).filter((d) => d[1]).length || weapon.system.damageTypeOverride)) {
   ...
   */
	...
	async roll({ configureDialog = true, messageMode = null, createMessage = true, variance = {} } = {}) {
		...
		const cardData = await (async () => {
			if (((this.type === "power") || (this.type === "consumable")) && this.system.autoGenChatPowerCard) {
				...
				let cardString = utils.preparePowerCardData(await this.getChatData({}, variance), rollData, attackBonus);
				return cardString;
			} else {
				return null;
			}
		...
		if (["tool", "ritual"].includes(this.type)) {
			...
			templateData.abilityCheck = utils.byString(this.system.attribute.replace(".mod", ".label").replace(".total", ".label"), this.actor?.system);
		}
		...
		if (["power", "consumable"].includes(templateData.item.type)) {
			...
			if (game.settings.get("dnd4e", "autoApplyEffects")) {
				if (typeof this.parent === "string") {
					...
					utils.applyEffectsToTokens(this.effects, [attacker.token], "self", attacker);
					utils.applyEffectsToTargets(this.effects, attacker);
				} else {
					utils.applyEffectsToTokens(this.effects, [this.parent.token], "self", this.parent);
					utils.applyEffectsToTargets(this.effects, this.parent);
				}
			}
		}
	...
	async toChat() {
		...
		const cardData = await (async () => {
			if (((this.type === "power") || (this.type === "consumable")) && this.system.autoGenChatPowerCard) {
				let cardString = utils.preparePowerCardData(await this.getChatData(), rollData);
	...
	rangeData() {
		...
		let area;
		if (this.system.area) {
			const areaForm = utils.evaluateFormula(String(this.system.area), this.actor?.getRollData(), { strict: true, contextName: "rangeData" });
		...
		if (this.system.rangeType === "range") {
			rangeData.rangeText = `${C.rangeType.range.label} ${utils.evaluateFormula(this.system.rangePower, actorData, { strict: true, context: "rangeText" })}`;
			...
			if (this.system.range.long) {
				rangeData.rangeText += `/${utils.evaluateFormula(this.system.range.long, actorData, { strict: true, contextName: "rangeText" })}`;
				rangeData.rangeTextBlock += `/${utils.evaluateFormula(this.system.range.long, actorData, { strict: true, contextName: "rangeText" })}`;
			}
			...
			if ((Roll.replaceFormulaData(this.system.rangePower, actorData) === undefined) || (Roll.replaceFormulaData(this.system.rangePower, actorData) === null)) {
				...
				rangeData.rangeText = `${C.rangeType.melee.label} ${utils.evaluateFormula(this.system.rangePower, actorData, { strict: true, context: "rangeText" })}`;
				rangeData.rangeTextBlock = `${utils.evaluateFormula(this.system.rangePower, actorData, { strict: true, context: "rangeTextBlock" })}`;
			}
		} else if (this.system.rangeType === "reach") {
			rangeData.rangeText = `${C.rangeType.reach.label} ${utils.evaluateFormula(this.system.rangePower, actorData, { strict: true, context: "rangeText" })}`;
			...
			try {
				...
				if (weaponUse.system.isRanged && (this.system.weaponType !== "melee")) {
					...
					rangeData.rangeTextBlock = `${utils.evaluateFormula(weaponUse.system.range.value, actorData, { strict: true, context: "rangeTextBlock" })}/${utils.evaluateFormula(weaponUse.system.range.long, actorData, { strict: true, context: "rangeTextBlock" })}`;
				} else {
	...
	async placeTemplate(options = {}) {
		...
		const shapes = Array.fromRange(shapeCount).map(() => {
			...
			for (const [key, path] of Object.entries(shapeProperties)) {
				shapeData[key] = utils.evaluateFormula(this.system[path], this.actor?.getRollData(), { strict: true, contextName: "templateData" }) * canvas.dimensions.distancePixels;
			}
	...
	async rollDamage({ event, spellLevel = null, fastForward = undefined, variance = {} } = {}) {
		...
		const secondaryPartsHelper = (formula, damageType) => {
			...
			options.formulaInnerData = foundry.utils.mergeObject(options.formulaInnerData, utils.getDataObject(formula, rollData));
		...
		Hooks.callAll("dnd4e.preDamageRoll", this, this.actor, { parts, partsMiss, partsCrit, partsExpressionReplacements, partsMissExpressionReplacement, partsCritExpressionReplacement, damageTypes: pD, options });
		...
		if (itemData.hit?.formula) {
			...
			const secondaryDamageExpressionHelper = (oldParts, expressionParts, newPartsArr) => {
				const newParts = newPartsArr.map(d => {
					options.formulaInnerData = foundry.utils.mergeObject(options.formulaInnerData, utils.getDataObject(d.formula, rollData));
			...
			if (weaponUse) {
				...
				if (itemData.miss.formula.includes("@wepDamage") && weaponUse.system.damage.parts.length) {
					secondaryDamageExpressionHelper(partsMiss, partsMissExpressionReplacement, weaponUse.system.damage.parts);
				}
				if (itemData.miss.formula.includes("@impDamage") && weaponUse.isActorImplementProficient && weaponUse.system.damageImp.parts.length) {
		...
		if (this.system?.hit?.damageBonusNull) utils.debugLog("Ignoring damage bonuses due to power config.");

		// Define Roll Data
		if (!this.system?.hit?.damageBonusNull) {
			const actorBonus = foundry.utils.getProperty(actorInnerData, `bonuses.${itemData.actionType}`) || {};
			if (actorBonus.damage && parseInt(actorBonus.damage)) {
				// parts.push("@dmg");
				// partsCrit.push("@dmg");
		...
		if (!this.system?.hit?.damageBonusNull) {
			await utils.applyEffects(rollData, actorData, itemData, weaponData, "damage", extraDamageParts, null, options);
		}
		...
		if (missDamageFormula.includes("@damageFormula")) {
			missDamageFormula = missDamageFormula.replace("@damageFormula", utils.bracketed(damageFormula));
		}

		if (missDamageFormula.includes("@halfDamageFormula")) {
			missDamageFormula = missDamageFormula.replace("@halfDamageFormula", utils.bracketed(`${damageFormula}/2`));
		}
	...
	rollHealing({ event, spellLevel = null, fastForward = undefined } = {}) {
		...
		const formulaHelper = (formula) => {
			// store the values that were used to sub in any formulas
			options.formulaInnerData = foundry.utils.mergeObject(options.formulaInnerData, utils.getDataObject(formula, actorData.getRollData(), rollData));
			// convert formula and type into a single string of "substituted formula [type]"
			return Roll.replaceFormulaData(formula, rollData);
		};
	...
	rollToolOrRitualCheck(rollType, titleKey, options = {}) {
		...
		if (this.system.formula) {
			rollData[rollType] = Roll.replaceFormulaData(this.system.formula.replace("@attribute", utils.byString(this.system.attribute, this.actor?.system)), this.getRollData());
		} else {
			rollData[rollType] = `1d20 + ${utils.byString(this.system.attribute, this.actor?.system)}`;
		...
		utils.debugLog(rollData[rollType]);
		const title = `${this.name} - ${_loc(titleKey)}`;

		const label = utils.byString(this.system.attribute.replace(".mod", ".label").replace(".total", ".label"), this.actor?.system);
	...
	getWepDice(weaponNum) {
		...
		const weaponData = this.type === "weapon" ? this.system : (this.type == "power" ? utils.getWeaponUse(this.system, this.actor)?.system : null);
	...
	getPowBase() {
		...
		if (diceType.includes("weapon")) {
			const weaponData = utils.getWeaponUse(this.system, this.actor)?.system;
			if (weaponData) {
				let parts = weaponData.damageDice.parts;
				for (let i = 0; i < parts.length; i++) {
	...
	getPowMax() {
		...
		if (utils.isNumber(rQuantity.total)) {
		...
		// Handle Weapon Type Damage
		if (diceType.includes("weapon")) {
			const weaponData = utils.getWeaponUse(powerData, this.actor)?.system;
			if (weaponData) {
				let parts = weaponData.damageDice.parts;
				for (let i = 0; i < parts.length; i++) {
					if (!parts[i].numDice || !parts[i].numFaces) continue;
	...
	getRollData(options = {}) {
		...
		if (weaponData) {
			...
			if (game.settings.get("dnd4e", "inhEnh")) {
				...
				enhValue = Math.max(enhValue, utils.findKeyScale(data.details.level, CONFIG.DND4E.SCALE.basic, 1));
				console.log(`Checked inherent atk/dmg enhancement of +${utils.findKeyScale(data.details.level, CONFIG.DND4E.SCALE.basic, 1)} for this level against weapon value of +${enhValue}`);
			}

			data.enhanceImp = ((weaponData.type === "implement") || weaponData.properties.imp) ? utils.bracketed(Roll.replaceFormulaData(String(enhValue), data, { recursive: true }) || 0) : 0;
			data.enhance = utils.bracketed(Roll.replaceFormulaData(String(enhValue), data, { recursive: true }) || 0);
			...
			if (weaponData.weaponType === "implement") {
				data.wepAttack = utils.bracketed(Roll.replaceFormulaData(weaponData.attackFormImp, data, { recursive: true }) || 0);
				data.wepDamage = utils.bracketed(Roll.replaceFormulaData(weaponData.damageFormImp, data, { recursive: true }) || 0);
				data.wepCritBonus = utils.bracketed(Roll.replaceFormulaData(weaponData.critDamageFormImp, data, { recursive: true }) || 0);
			}
			else {
				data.wepAttack = utils.bracketed(Roll.replaceFormulaData(weaponData.attackForm, data, { recursive: true }) || 0);
				data.wepDamage = utils.bracketed(Roll.replaceFormulaData(weaponData.damageForm, data, { recursive: true }) || 0);
				data.wepCritBonus = utils.bracketed(Roll.replaceFormulaData(weaponData.critDamageForm, data, { recursive: true }) || 0);
			}

			data.impCritBonus = utils.bracketed(Roll.replaceFormulaData(weaponData.critDamageFormImp, data, { recursive: true }) || 0);

			data.impAttackO = utils.bracketed(Roll.replaceFormulaData(weaponData.attackFormImp, data, { recursive: true }) || 0);
			data.impDamageO = utils.bracketed(Roll.replaceFormulaData(weaponData.damageFormImp, data, { recursive: true }) || 0);

			data.impAttack = utils.bracketed(weapon.isActorImplementProficient ? Roll.replaceFormulaData(weaponData.attackFormImp, data, { recursive: true }) || 0 : 0);
			data.impDamage = utils.bracketed(weapon.isActorImplementProficient ? Roll.replaceFormulaData(weaponData.damageFormImp, data, { recursive: true }) || 0 : 0);
		} else {
			data.profBonusO = 0;
	...
	static async _onChatCardAction(event) {
		...
		// Effects
			...
			utils.applyEffectsToTokens([effect], targets, effect.system.powerEffectType, actor);
		}
		else if (action === "effect") utils.applyAllXEffectsToTokens(item.effects, actor, effectTargets);
		else if (action === "hitEffect") utils.applyEffectsToTokens(item.effects, effectTargets, "hit", actor);
		else if (action === "missEffect") utils.applyEffectsToTokens(item.effects, effectTargets, "miss", actor);
		else if (action === "hitOrMissEffect") utils.applyEffectsToTokens(item.effects, effectTargets, "hitOrMiss", actor);
	 ...
	 * @type {Item4e|Promise<Item4e>|null}
	 */
	get container() {
